  ...
  <div>
    <div class="container">
      <div class="handle-box">
        <el-form :model="query.form">
          ...
          <el-row :gutter="20" class="handle-el-row">
            <el-col :span="8">
              <el-form-item label="免疫名称" :label-width="formLabelWidth">
                <el-select v-model="query.form.immunityName" filterable style="width:100%" placeholder="请选择">
                  <el-option label="全部" value=""></el-option>
                  <el-option v-for="item in listImmunityName"
                             :key="item.key"
                             :label="item.value"
                             :value="item.key">
                  </el-option>
            ...
            <el-col :span="8">
              <el-form-item label="免疫方式" :label-width="formLabelWidth">
                <el-select v-model="query.form.immunityMethod" filterable style="width:100%" placeholder="请选择">
                  <el-option label="全部" value=""></el-option>
                  <el-option v-for="item in listImmunityMethod"
                             :key="item.key"
                             :label="item.value"
                             :value="item.key">
                  </el-option>
            ...
            <el-col :span="8">
              <el-form-item label="药品名称" :label-width="formLabelWidth">
                <el-input v-model="query.form.drugName" placeholder="请输入"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20" class="handle-el-row">
            <el-col :span="8">
              <el-form-item label="剂量" :label-width="formLabelWidth">
                <el-input v-model="query.form.dose" placeholder="请输入"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
      ...
      >
        ...
        <el-table-column prop="immunityNameValue" label="免疫名称"></el-table-column>
        <el-table-column prop="immunityMethodValue" label="免疫方式"></el-table-column>
    ...
    <el-dialog :destroy-on-close="true" title="新增" :visible.sync="saveDialog.visible">
      <el-form :model="saveDialog.form" ref="saveDialog.form" :rules="saveDialog.rules">
        ...
        <el-form-item label="免疫名称" :label-width="formLabelWidth" prop="immunityName">
          <el-select v-model="saveDialog.form.immunityName" filterable style="width:100%" placeholder="请选择">
            <el-option v-for="item in listImmunityName"
                       :key="item.key"
                       :label="item.value"
                       :value="item.key">
        ...
        <el-form-item label="免疫方式" :label-width="formLabelWidth" prop="immunityMethod">
          <el-select v-model="saveDialog.form.immunityMethod" filterable style="width:100%" placeholder="请选择">
            <el-option v-for="item in listImmunityMethod"
                       :key="item.key"
                       :label="item.value"
                       :value="item.key">
        ...
        <el-form-item label="药品名称" :label-width="formLabelWidth" prop="drugName">
          <el-input v-model="saveDialog.form.drugName" placeholder="请输入"></el-input>
        </el-form-item>
        <el-form-item label="剂量" :label-width="formLabelWidth" prop="dose">
          <el-input v-model="saveDialog.form.dose" placeholder="请输入"></el-input>
        </el-form-item>
...
import {pageImmunityRegister, addImmunityRegister, delImmunityRegister} from '@/api/healthDisease';
import {listUser} from '@/api/user';
import {listFarmZone} from "@/api/farm";
import {listCattle} from "@/api/cattle";

export default {
  name: 'ImmunityRegister',
  components: {
    ImportExport,
    UserInfo,
    CattleInfo
    ...
    return {
      ...
      saveDialog: {
        ...
        rules: {
          ...
          immunityName: [{required: true, message: '免疫名称不能为空', trigger: 'change'}],
          immunityMethod: [{required: true, message: '免疫方式不能为空', trigger: 'change'}],
          drugName: [{required: true, message: '药品名称不能为空', trigger: 'change'}],
          dose: [{required: true, message: '剂量不能为空', trigger: 'change'}],
  ...
  created() {
    this.power = getPageActionPower('immunityRegister');
    listSysConfig('immunityName').then(res => this.listImmunityName = res);
    listSysConfig('immunityMethod').then(res => this.listImmunityMethod = res);
  ...
  methods: {
    ...
    getData() {
      ...
      this.loading = true;
      pageImmunityRegister(this.queryParams).then(res => {
    ...
    saveInfo() {
      this.$refs['saveDialog.form'].validate((valid) => {
        ...
        addImmunityRegister(data).then(res => {
          if (res > 0) {
    ...
    delInfo() {
      ...
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        delImmunityRegister(this.multipleSelection.map(item => item.id)).then(res => {
          if (res > 0) {
